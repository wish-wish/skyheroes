...
function AppBase:ctor(configs,path)
    self.configs_ = {
        viewsRoot  = string.format("apps.%s.views",path),
        modelsRoot = string.format("apps.%s.models",path),
...
function AppBase:enterScene(sceneName, transition, time, more)
    ...
    if running then
        print("add view:"..sceneName);
        running:addChild(view);
    else
        print("run scene:"..sceneName);
