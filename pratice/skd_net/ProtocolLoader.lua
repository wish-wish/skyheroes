...
local ProtocolLoader = class("ProtocolLoader");

local ByteArray=require("framework.cc.utils.ByteArray");
local ByteArrayVariant=require("framework.cc.utils.ByteArrayVarint");
local Protocols=require("app.skd_net.skydream_protocol");

function ProtocolLoader:cloneTab(atab)
    if atab==nil then
        return nil;
    end 
    local tab={};
...
function cloneTab(atab)
    return ProtocolLoader:cloneTab(atab);
end

function ProtocolLoader:encode(atab,bytearr)
    
    if atab.define==nil then
        return nil;
    end    
    for k,field_desc in ipairs(atab.define) do
...
function ProtocolLoader:isSpecialType(cmd_id)
...
function ProtocolLoader:decode(cmd_id,bytearr,alen,tlen,typeflag)
    local struct=nil;
    ...
    if struct==nil then
        skdLog():logTFStr(cmd_id.." is nil");
        return nil;
    end
    local alen=alen or tlen;
    for k,field_desc in ipairs(struct.define) do        
        local basetype=field_desc[2];
        local blocktype=field_desc[3];
        if blocktype<20 then 
            if alen<field_desc[5]-1 then  --TODO:特殊处理，服务要求
                skdLog():logTFStr("%s for keys cmdid %d all len %d",field_desc[1],cmd_id,alen or -1);
                return struct;
            end
        end
        local ttlen=tlen;
        if bytearr:getPos()>ttlen then --TODO:特殊处理，服务要求
            skdLog():logTFStr("%s for keys cmdid %d pos %d blen %d",field_desc[1],cmd_id,bytearr:getPos(),ttlen or -1);
            return struct;
        end
        ...
        local function BaseTypeRead()                        
            if alen<field_desc[5] then --TODO:特殊处理，服务要求
                skdLog():logTFStr("%s BaseTypeRead cmdid %d all len %d pos %d blen %d",field_desc[1],cmd_id,alen,bytearr:getPos(),ttlen);
                return struct;
            end            
            local ret=nil;
            if basetype=="bit" then
        ...
        if blocktype>19 then --array
            if blocktype==20 then --base array
                if basetype=="INT8" or basetype=="UINT8" then               
                    ...
                    if flen>alen then
                        skdLog():logTFStr("%s read StringLen %d alen %d",field_desc[1],flen,alen);
                        flen=alen;
                    end
                    --local flen=field_desc[5];
            ...
            elseif blocktype==22 then --struct array                                
                local iflen=field_desc[5];
                if iflen==0 then
                    struct_count=struct[struct.define[k-1][1]];
                else
...
function ProtocolLoader:qryTypeProtocol(type_str)
    return self:cloneTab(Protocols[type_str]);
end

function ProtocolLoader:qryStrProtocol(cmd_str)
...
function ProtocolLoader:qryIdProtocol(cmd_id)
    for k,v in pairs(Protocols) do
...
function ProtocolLoader:qryCmdID(cmd_str)
    return skdCmds()[cmd_str];
end

function ProtocolLoader:qryCmdStr(cmd_id)
