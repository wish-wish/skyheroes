...
function ByteArray:writeBit(_byte,start,len)
    --TODO:write bit to bytearray    
end

function ByteArray:parseBit(_byte,start,len)    
    local bits=bit.tobits(_byte);
    local bitsn={};
    for i=start,start+len do
        if bits[i]~=nil then
            table.insert(bitsn,bits[i]);
        else
            table.insert(bitsn,0);
        end
    end    
    local num=bbit:tonumb(bitsn);
    return num;
end
