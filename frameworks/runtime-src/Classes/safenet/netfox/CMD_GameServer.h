...
struct CMD_GR_UserSitDown
{
	WORD							wTableID;							//桌子位置
	WORD							wChairID;							//椅子位置
	char							szPassword[LEN_PASSWORD];			//桌子密码
};
...
struct CMD_GR_C_SendTrumpet
{
	...
	char                          szTrumpetContent[TRUMPET_MAX_CHAR];  //喇叭内容
};
...
struct CMD_GR_C_TakeScoreRequest
{
	...
	char							szInsurePass[LEN_PASSWORD];			//银行密码
};
...
struct CMD_GR_SendWarning
{
	WORD							wChatLength;						//信息长度
	DWORD							dwTargetUserID;						//目标用户
	char							szWarningMessage[LEN_USER_CHAT];	//警告消息
};
...
struct CMD_GR_SendMessage
{
	...
	BYTE							cbAllRoom;							//游戏消息
	WORD							wChatLength;						//信息长度
	char							szSystemMessage[LEN_USER_CHAT];		//系统消息
};
...
{
	char							szKickMessage[LEN_USER_CHAT];		//踢出提示
};
...
{
	char							szTitle[MAX_MATCH_DESC][16];		//信息标题
	char							szDescribe[MAX_MATCH_DESC][64];		//描述内容
