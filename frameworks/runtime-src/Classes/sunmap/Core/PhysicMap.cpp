...
void PhysicMap::setBitmap( Image* pTexture, int x, int y )
{
...
void PhysicMap::update( Image* pTexture, RawTile* pTile )
{
	bool bIn = false;
...
Image* PhysicMap::getCell( int x, int y )
{
	Image* pTexture =m_Cells[y*m_nTotalCellsX+x];
	return pTexture;
}

Vec2 PhysicMap::getGlobalOffset()
{
...
void PhysicMap::setGlobalOffset(Vec2 pnt)
{
...
bool PhysicMap::quickHack(bool bForce)
{
	int dx = 0, dy = 0;
	int tdx = 0, tdy = 0;
	Vec2 globalOffset = getGlobalOffset();
...
{
	m_pBg = new Image;
	m_pBg->initWithImageFile("geo/bg.png");
}

int PhysicMap::zoomS( double dz )
{
