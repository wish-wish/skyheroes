...
int SimulatorWin::run()
{
	...
	if (CC_TARGET_PLATFORM == CC_PLATFORM_WIN32 || CC_TARGET_PLATFORM == CC_PLATFORM_WINRT || CC_TARGET_PLATFORM == CC_PLATFORM_MAC || CC_PLATFORM_MAC == CC_PLATFORM_LINUX)
	{
		...
		if (SystemParametersInfo(SPI_GETWORKAREA, NULL, &workareaSize, NULL))
		{
			...
			if (frameScale < 0.4f) frameScale = 0.4f;
		}
	...
	{
		RECT workareaSize;
		SystemParametersInfo(SPI_GETWORKAREA, NULL, &workareaSize, NULL);
		RECT rect;
		GetWindowRect(_hwnd, &rect);
		Vec2 center = Vec2((workareaSize.right - workareaSize.left - (rect.right - rect.left)) / 2, (workareaSize.bottom - workareaSize.top - (rect.bottom - rect.top)) / 2);
		Vec2 right = Vec2(workareaSize.right - (rect.right - rect.left) - 100, center.y);
		#ifdef _DEBUG
			center.x = right.x;
		#endif
		MoveWindow(_hwnd, center.x, center.y, rect.right - rect.left, rect.bottom - rect.top, FALSE);
	}
