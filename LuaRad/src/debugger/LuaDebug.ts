...
export class LuaDebug extends DebugSession {
	...
	runInTerminalRequest(args: DebugProtocol.RunInTerminalRequestArguments, timeout: number, cb: (response: DebugProtocol.RunInTerminalResponse) => void){
		this.luaStartProc.kill();
	};
	*/
	...
	protected launchRequest(response: DebugProtocol.LaunchResponse, args: any): void {
		//TODO:auto copy debug && debugjit to localRoot/src and modify the mian.lua
		//var luaRadExtension = vscode.extensions.getExtension(ConstInfo.extensionLuaRadConfig);
		var dir=getConfigDir();
		var configpath=path.join(dir,ConstInfo.luaradConfig);
		var extpath=fs.readFileSync(configpath, 'utf-8');
		var localluadebugf=path.join(args.localRoot,"/src/LuaDebug.lua");
		var isExists=true;
		if (!fs.existsSync(localluadebugf)) {
			var luadebugf=path.join(extpath,"/luadebug/LuaDebug.lua");
			var dText = fs.readFileSync(luadebugf, 'utf-8');
			fs.writeFileSync(localluadebugf,dText);
			isExists=false;
		}
		var localluajitbugf=path.join(args.localRoot,"/src/LuaDebugjit.lua");
		if (!fs.existsSync(localluajitbugf)) {
			var luadejitbugf=path.join(extpath,"/luadebug/LuaDebugjit.lua");
			var djitText = fs.readFileSync(luadejitbugf, 'utf-8');
			fs.writeFileSync(localluajitbugf,djitText);
			isExists=false;
		}
		if(!isExists)
		{
			var localmain=path.join(args.localRoot,"/src/_luarad_mian_.lua");
			if (!fs.existsSync(localmain)&&!fs.existsSync(path.join(args.localRoot,"/src/_mian_.lua"))) {
				var debugmain=path.join(extpath,"/luadebug/main.lua");
				var mainText = fs.readFileSync(debugmain, 'utf-8');
				fs.writeFileSync(localmain,mainText);
			}
		}
		...
		this.luaStartProc.on('close', function (code) {	
			try {
				...
				luadebug.sendEvent(new OutputEvent('error:'+e.toString()));
			}
			luadebug.sendEvent(new OutputEvent("sim-close" + "\n"));
	...
	protected disconnectRequest(response: DebugProtocol.DisconnectResponse, args: DebugProtocol.DisconnectArguments): void {
		...
		if (this.luaStartProc) {
			this.luaStartProc.kill("0");
			//this.sendEvent(new OutputEvent("kill luaStartProc "+this.luaStartProc.killed + "\n"))
            delete this.luaStartProc;
        }
        if (this.luaProcess) {
			//this.sendEvent(new OutputEvent("close luaProcess" + "\n"))
			this.luaProcess.close();
            delete this.luaProcess;
		}
