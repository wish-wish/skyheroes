...
export class LuaRadConfigManager {
    ...
    constructor() {
        ...
        try{
            ...
            //vscode.window.showInformationMessage(ConstInfo.extensionName + "启动失败,请检查<"+err+">"+this.extensionPath);
            console.log(err);
        }
    }
    public showIndex() {
        ...
        if (this.userInfo.showIndex == 0 || this.isShowDest) {
            ...
            vscode.commands.executeCommand('vscode.previewHtml', previewUri, vscode.ViewColumn.Three, "LuaRad介绍").then(value => {
            })
    ...
    public configInit() {
        var luaradConfig: vscode.WorkspaceConfiguration = vscode.workspace.getConfiguration("LuaRad")
        var macroListConfig: Array<any> = luaradConfig.get<Array<any>>("macroList");
        this.luaOperatorCheck = luaradConfig.get<boolean>("luaOperatorCheck")
        this.luaFunArgCheck = luaradConfig.get<boolean>("luaFunArgCheck")
        this.isShowDest = luaradConfig.get<boolean>("isShowDest")
        this.changeTextCheck = luaradConfig.get<boolean>("changeTextCheck")
        this.moduleFunNestingCheck = luaradConfig.get<boolean>("moduleFunNestingCheck")
        this.maxFileSize = luaradConfig.get<number>("maxFileSize")
        this.showOnLine = luaradConfig.get<boolean>("showOnLine");
        var scriptRoots: Array<string> = luaradConfig.get<Array<string>>("scriptRoots");
        ...
        if(this.scriptRoots.length == 0){
             vscode.window.showInformationMessage("请在 文件->首选项->设置->工作区设置 添加 LuaRad.scriptRoots 的配置,否则无法获得最好的提示代码联想效果!",{
                title: "配置项文档",
        ...
        */
        ...
        var luaTemplatesDir: string = luaradConfig.get<string>("luaTemplatesDir")
        if (luaTemplatesDir) {
    ...
    public showRecharge() {
        ...
        if (
            ...
            vscode.window.showInformationMessage("您愿意赞助LuaRad吗?", {
                title: "支持一个",
                isCloseAffordance: true,
                id: 1
            }, {
    ...
    private copyConfig() {
        ...
        var configFile = path.join(userPath, "luaradConfig")
