...
export class FileCompletionItemManager {
    ...
    public addCompletionItem(lp: LuaParse, luaInfo: LuaInfo, token: TokenInfo, tokens: Array<TokenInfo>,
        ...
        if (isFun) {
        ...
        } else if (this.currentFunctionNames!=null&&this.currentFunctionNames.length == 0) {
            if (luaInfo.isLocal) {
            ...
            if (data) {
                ...
                if (luaInfo.isLocal == false) {
                    ...
                    if (key != "") {
                        //判断是否为参数
                        if(this.currentFunctionParams!=null)
                        {
        ...
        if(infos!=null)
        {
            for (var i = forindex; i < infos.length; i++) {
                ...
                if(i == infos.length-1){
                    if(completion.isNewVar == false &&tokens!=null && endIndex+2 < tokens.length ){
        ...
        if (currentToken) {
            if (currentToken.type == TokenTypes.Punctuator && currentToken.value == "=") {
                ...
                if (currentToken.type == TokenTypes.Identifier) {
                    ...
                    if ( ExtensionManager.em.luaRadConfigManager.requireFunNames.indexOf(currentToken.value) > -1 ) {
                        //require 模式
                        index++;
                        currentToken = this.getValueToken(index, tokens)
                        if (currentToken) {
