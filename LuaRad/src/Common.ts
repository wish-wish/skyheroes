...
export function initConfig(args: any): any {
    ...
    if (extensionPath == false) {
        return "未能获取 luarad 路径! "+localRoot
    }
    if (runtimeType == "Lua51") {
        ...
        //if(args.exePath)
        var exepath=args.exePath;
        switch (process.platform) {
            case 'win32':
                var params=exepath.split(":");
                if(params.length==1)
                {
                    exepath=path.join(args.localRoot,exepath);
        ...
        if (!fs.existsSync(args.exePath)) {
            return `File does not exist. "${exepath}"`+args.localRoot;
        }
        if (args.scripts) {
...
export function getExtensionPath() {
    ...
    var configFile: string = path.join(dir, "luaradConfig")
...
export function getConfigDir() {
    ...
    userDataDir = path.join(userDataDir, 'code', 'user', "luarad");
